jQuery(document).ready( function( $ ) {
	...
	$( document ).on( 'click', '.trobica_tab_reset_settings', function() {

		var reset_message = trobica_admin_meta.reset_message;
		...
		if( flag ) {
			...
			var reset_tab = $( this ).parents( '.trobica_meta_box_tab' );
	        ...
	        reset_tab.find( '.trobica_remove_button, .multiple_images .remove' ).trigger( 'click' );
	    }
	});

	// Check on load for selected tab when user come before if not it show first one active
	if($.cookie('trobica_metabox_active_id_' + $('#post_ID').val())) {
		var active_class = $.cookie('trobica_metabox_active_id_' + $('#post_ID').val());

		$('#trobica_admin_options').find('.trobica_meta_box_tabs li').removeClass('active');
		$('#trobica_admin_options').find('.trobica_meta_box_tab').removeClass('active').hide();

		$('.'+active_class).addClass('active').fadeIn();
		$('#trobica_admin_options').find('#'+active_class).addClass('active').fadeIn();

	} else {
		$('.trobica_meta_box_tabs li:first-child').addClass('active');
		$('.trobica_meta_box_tab_content .trobica_meta_box_tab:first-child').addClass('active').fadeIn();
	}
	$('.trobica_meta_box_tabs li a').click(function(e) {
		e.preventDefault();

		var tab_click_id = $(this).parent().attr('class').split(' ')[0];
		var tab_main_div = $(this).parents('#trobica_admin_options');

		$.cookie('trobica_metabox_active_id_' + $('#post_ID').val(), tab_click_id, { expires: 7 });
		
		tab_main_div.find('.trobica_meta_box_tabs li').removeClass('active');
		tab_main_div.find('.trobica_meta_box_tab').removeClass('active').hide();
  ...
  /* Metabox dependance of fields */
    ...
    $(".trobica_select_parent").change(function () {
	    var str_selected = $(this).find("option:selected").val();
	    var tab_active_status_main = $(this).parents('#trobica_admin_options');
		...
		if(str_selected == 'trobica_layout_both_sidebar'){
			$('.trobica_layout_left_sidebar_single').removeClass('hide_dependency');
			$('.trobica_layout_left_sidebar_single').find('input[type="hidden"]').val('1');
			$('.trobica_layout_right_sidebar_single').removeClass('hide_dependency');
			$('.trobica_layout_right_sidebar_single').find('input[type="hidden"]').val('1');
		}
    ...
    if( $( 'body' ).hasClass('post-type-portfolio') ) {
		...
		$( '#trobica_portfolio_post_type_single' ).val( defaultPortfolioSelect );
		$( '.post-type-portfolio').on( 'click', '.post-format', function() {
			var clickVal = $(this).val();
			if( clickVal == 'link' ){
				$( '#trobica_portfolio_post_type_single' ).val( clickVal );
			} else {
				$( '#trobica_portfolio_post_type_single' ).val( 'standard' );
			}
		});
	}

    var link_color = $( '.trobica-color-picker' );
    ...
    $( document ).on( 'click', '.trobica_upload_button', function(event) {
          var file_frame;
        var button = $(this);
    ...
    $( document ).on( 'click', '.trobica_remove_button', function(event) {
    ...
    /* multiple image upload */
      ...
      $( document ).on( 'click', '.trobica_upload_button_multiple', function(event) {
            var file_frame;
          var button = $(this);
	...
	function post_format_selection_options() {
			...
			if ($('#post-format-link').is(':checked')) {
				$('.trobica_portfolio_external_link_single_box').fadeIn();
			}else{
				$('.trobica_portfolio_external_link_single_box').hide();
			}
			
			$('body.post-type-post #trobica_admin_options_single').hide();

	       if ($('#post-format-gallery').is(':checked')) {
	        	$('body.post-type-post #trobica_admin_options_single').show();
	            $('.trobica_gallery_single_box').fadeIn();
	            $('.trobica_lightbox_image_single_box').fadeIn();
	            $('.trobica_quote_single_box').hide();
	            $('.trobica_link_type_single_box').hide();
	            $('.trobica_link_single_box').hide();
	            $('.trobica_video_mp4_single_box').hide();
	            $('.trobica_video_ogg_single_box').hide();
	            $('.trobica_video_webm_single_box').hide();
	            $('.trobica_video_single_box').hide();
	            $('.trobica_video_type_single_box').hide();
	            $('.trobica_audio_single_box').hide();
	            $('.trobica_enable_mute_single_box').hide();
	            $('.trobica_featured_image_single_box').fadeIn();
	            $('.trobica_subtitle_single_box').fadeIn();
	            $(".trobica_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
				$(".trobica_video_mp4_single_box").removeClass('show_div').removeClass('hide_div');
				$(".trobica_video_ogg_single_box").removeClass('show_div').removeClass('hide_div');
				$(".trobica_video_webm_single_box").removeClass('show_div').removeClass('hide_div');
				$(".trobica_video_single_box").removeClass('show_div').removeClass('hide_div');

	        } else if ($('#post-format-video').is(':checked')) {
	        	$('body.post-type-post #trobica_admin_options_single').show();
	            $('.trobica_gallery_single_box').hide();
	            $('.trobica_lightbox_image_single_box').hide();
	            $('.trobica_quote_single_box').hide();
	            $('.trobica_link_type_single_box').hide();
	            $('.trobica_link_single_box').hide();
	            $('.trobica_video_mp4_single_box').fadeIn();
	            $('.trobica_video_ogg_single_box').fadeIn();
	            $('.trobica_video_webm_single_box').fadeIn();
	            $('.trobica_video_single_box').fadeIn();
	            $('.trobica_video_type_single_box').fadeIn();
	            $('.trobica_audio_single_box').hide();
	            $('.trobica_enable_mute_single_box').fadeIn();
	            $('.trobica_featured_image_single_box').fadeIn();
	            $('.trobica_subtitle_single_box').fadeIn();
	            ...
	            $('.trobica_video_single_box').hide();
	            $('.trobica_video_type_single_box').hide();
	            $('.trobica_audio_single_box').fadeIn();
	            $('.trobica_enable_mute_single_box').hide();
	            $('.trobica_featured_image_single_box').fadeIn();
	            $('.trobica_subtitle_single_box').fadeIn();
	            $(".trobica_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
	            ...
	            $('.trobica_gallery_single_box').hide();
	            $('.trobica_lightbox_image_single_box').hide();
	            $('.trobica_quote_single_box').fadeIn();
	            $('.trobica_link_type_single_box').hide();
	            $('.trobica_link_single_box').hide();
	            $('.trobica_video_mp4_single_box').hide();
	            $('.trobica_video_ogg_single_box').hide();
	            ...
	            $('.trobica_lightbox_image_single_box').hide();
	            $('.trobica_quote_single_box').hide();
	            $('.trobica_image_single_box').fadeIn();
	            $('.trobica_link_type_single_box').hide();
	            $('.trobica_link_single_box').hide();
	            $('.trobica_video_mp4_single_box').hide();
	            $('.trobica_video_ogg_single_box').hide();
	...
	$('#trobica_video_type_single').change(function() {
		post_format_video_selection();
	});

	function post_format_video_selection(){
		if( $('#trobica_video_type_single').val() == 'self' && $('#post-format-video').is(':checked') ){
			$(".trobica_enable_mute_single_box").addClass('show_div').removeClass('hide_div');
			$(".trobica_video_mp4_single_box").addClass('show_div').removeClass('hide_div');
			$(".trobica_video_ogg_single_box").addClass('show_div').removeClass('hide_div');
			$(".trobica_video_webm_single_box").addClass('show_div').removeClass('hide_div');
			$(".trobica_video_single_box").removeClass('show_div').addClass('hide_div');
		} else if( $('#trobica_video_type_single').val() == 'external' && $('#post-format-video').is(':checked') ){
			$(".trobica_enable_mute_single_box").removeClass('show_div').addClass('hide_div');
			$(".trobica_video_mp4_single_box").removeClass('show_div').addClass('hide_div');
			$(".trobica_video_ogg_single_box").removeClass('show_div').addClass('hide_div');
			$(".trobica_video_webm_single_box").removeClass('show_div').addClass('hide_div');
			$(".trobica_video_single_box").addClass('show_div').removeClass('hide_div');
		} else{
			$(".trobica_enable_mute_single_box").removeClass('show_div').removeClass('hide_div');
			$(".trobica_video_mp4_single_box").removeClass('show_div').removeClass('hide_div');
			$(".trobica_video_ogg_single_box").removeClass('show_div').removeClass('hide_div');
