...
get_header(); 
	...
	//gallery at top
	if ( get_post_meta($post->ID, 'trobica_port_format', true) == 'port_standard' ){ ?>
	...
	<?php }  else { 
		if ( get_post_meta($post->ID, 'trobica_port_format', true) == 'port_two' && get_post_meta($post->ID, 'trobica_top_type', true) == 'top_content_youtube' ){ ?>
			  <!--YOUTUBE BACKGROUND-->
			  <div class="bg-youtube" data-property="{videoURL:'https://www.youtube.com/watch?v=<?php echo esc_attr(get_post_meta($post->ID, 'trobica_port_youtube_link', true)); ?>', opacity:1, 
			  ...
			  quality:'<?php if ( get_post_meta($post->ID, 'trobica_port_youtube_quality', true) != '') {echo esc_attr(get_post_meta($post->ID, 'trobica_port_youtube_quality', true)); } else {echo 'large';} ?>',
			   realfullscreen:'true', ratio:'auto'}"></div>
			  <!--YOUTUBE BACKGROUND END-->
		<?php } else if ( get_post_meta($post->ID, 'trobica_port_format', true) == 'port_two' && get_post_meta($post->ID, 'trobica_top_type', true) == 'top_content_video' ){ ?>
			
			<div class="bg-vid"  data-link="<?php echo esc_url(get_post_meta($post->ID, 'trobica_port_video_link', true)); ?>"  data-ambient="true"></div>
		<?php } ?>
			...
			<div class="home-video-box">
				...
				<div class="home-slider ani-slider slider" data-slick='{"autoplaySpeed": 8000}'>
					<div class="slide">
						...
						<div class="<?php if ( get_post_meta($post->ID, 'trobica_port_format', true) == 'port_two' && get_post_meta($post->ID, 'trobica_top_type', true) == 'top_content_slider' ){ 
							echo 'work-img-bg'; } ?> slider-img-bg" data-animation="puffIn" data-delay="0.2s" data-animation-duration="0.7s"data-background="<?php echo esc_url(get_post_meta($post->ID, 'trobica_port_slider_setting', true)); ?>"></div>
			...
			<div class="content">
				<div class="container clearfix">
					<div class="row">
						<div class="<?php if ( get_post_meta($post->ID, 'trobica_gallery_port_img', true) == ''){echo 'col-lg-12';}else{echo 'col-lg-8';} ?> ">
							<?php the_content ();?>
							<?php if ( get_post_meta($post->ID, 'port_item_btn_text', true) !='' && get_post_meta($post->ID, 'port_item_btn_link', true) != ''){?>
							<div class="spc-40 clearfix"></div>
							<a class="content-btn" href="<?php echo  esc_url(get_post_meta($post->ID, 'port_item_btn_link', true)); ?>">
						...
						<?php if ( get_post_meta($post->ID, 'trobica_gallery_port_img', true) != ''){?>
						...
						<div class="col-lg-4">
							<div class="port-gallery-body">
								...
								<?php /* get the gallery list array */
					  			$lists = get_post_meta($post->ID, 'gallery_list',  true);
								?>
								<div class="port-item col-md-12">
									<div class="port-inner">
									  <div class="port-box"></div>
									  <div class="port-img width-img img-bg" data-background="<?php echo esc_url(get_post_meta($post->ID,'trobica_gallery_port_img', true)); ?>"></div>
									</div><!--/.port-inner-->
								</div><!--.port-item-->
								<div class="spc-40"></div>
							</div><!--/.port-gallery-body-->
		...
		<div class="content gray-bg">   
			<div class="container-fluid">
				<div class="other-portfolio clearfix">
					<p class="op-sub"><?php if ( class_exists('ReduxFrameworkPlugin')&& trobica_option( 'trobica_other_port_sub' ) !=''){ echo  esc_html ( trobica_option( 'trobica_other_port_sub' )); } else {esc_html_e('See our other portfolio','trobica');} ?>
					</p>
					<h3 class="op-title"><?php if ( class_exists('ReduxFrameworkPlugin')&& trobica_option( 'trobica_other_port_title' ) !=''){ echo esc_html ( trobica_option( 'trobica_other_port_title' )); } else {esc_html_e('Other portfolio','trobica');} ?>
					</h3>
