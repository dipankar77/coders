...
<nav class="header apply-header not-custom-menu clearfix 
	<?php if ( is_singular() && get_post_meta($post->ID, 'trobica_header_format', true)!= 'head_trans'){ 
		echo 'white-header';
	} else { 
				...
				<div class="top-logo">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
						<?php if ( is_singular() && get_post_meta($post->ID, 'trobica_header_format', true)!= 'head_trans'){ ?>
							...
							<img alt="<?php esc_attr_e ('Logo','trobica'); ?>" class="logo1 logo-dark" src="<?php 
								if ( is_singular() && class_exists('ReduxFrameworkPlugin')&& trobica_option('trobica_header_logo_dark') ) {
									$trobica_header_logo_dark = trobica_option('trobica_header_logo_dark');
									echo esc_url ( $trobica_header_logo_dark['url']); 
								} else { 
									echo get_template_directory_uri(); ?>/images/logo.png <?php 
								} ?>"> 

							<img alt="<?php esc_attr_e ('Logo','trobica'); ?>" class="logo1 logo-white" src="<?php 
								if ( is_singular() && class_exists('ReduxFrameworkPlugin')&& trobica_option('trobica_header_logo_white') ) {
									$trobica_header_logo_white = trobica_option('trobica_header_logo_white');
									echo esc_url ( $trobica_header_logo_white['url']); 
								} else { 
									echo get_template_directory_uri(); ?>/images/logo-white.png <?php 
